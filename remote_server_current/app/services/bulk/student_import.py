"""
app/services/bulk/student_import.py
=====================================
Bulk import for Student records.
Creates User(role=STUDENT) + StudentProfile for new entries.
Duplicate check by email or enrollment_number.
"""
from __future__ import annotations

import uuid

from sqlalchemy import select
from sqlalchemy.ext.asyncio import AsyncSession

from app.core.upload import STUDENT_DEFAULT_PASSWORD
from app.models.student import StudentProfile
from app.models.user import User, UserRole
from app.schemas.bulk_upload import (
    BulkImportResult,
    BulkUploadPreview,
    BulkUploadRow,
    BulkRowStatus,
)
from app.services.bulk.base import (
    coerce_int,
    map_columns,
    resolve_department,
)

REQUIRED_FIELDS = ["name", "email", "enrollment_number", "department_id", "degree_type", "program"]


async def preview_import(
    db: AsyncSession,
    rows: list[dict[str, str]],
    institution_id: uuid.UUID,
) -> BulkUploadPreview:
    if not rows:
        return BulkUploadPreview(total_rows=0, to_create=[], to_update=[], errors=[])

    column_map = map_columns(list(rows[0].keys()))
    to_create: list[BulkUploadRow] = []
    to_update: list[BulkUploadRow] = []
    errors: list[BulkUploadRow] = []
    dept_cache: dict[str, uuid.UUID | None] = {}

    for idx, raw in enumerate(rows):
        row_num = idx + 2
        mapped = {field: raw.get(col, "") for col, field in column_map.items()}
        row_errors: list[str] = []

        for f in REQUIRED_FIELDS:
            if not mapped.get(f):
                row_errors.append(f"Missing required field: {f}")

        dept_id = None
        if mapped.get("department_id"):
            dept_id = await resolve_department(db, mapped["department_id"], institution_id, dept_cache)
            if not dept_id:
                row_errors.append(f"Department '{mapped['department_id']}' not found")

        # Check existing by email or enrollment_number
        existing = None
        if not row_errors:
            email = mapped.get("email", "")
            enroll = mapped.get("enrollment_number", "")
            if email:
                result = await db.execute(
                    select(StudentProfile)
                    .join(User, StudentProfile.user_id == User.id)
                    .where(User.email.ilike(email))
                )
                existing = result.scalar_one_or_none()
            if not existing and enroll:
                result = await db.execute(
                    select(StudentProfile).where(StudentProfile.enrollment_number.ilike(enroll))
                )
                existing = result.scalar_one_or_none()

        data = {
            "name": mapped.get("name", ""),
            "email": mapped.get("email", ""),
            "enrollment_number": mapped.get("enrollment_number", ""),
            "department_id": str(dept_id) if dept_id else "",
            "degree_type": mapped.get("degree_type", ""),
            "program": mapped.get("program", ""),
        }
        if mapped.get("phone"):
            data["phone"] = mapped["phone"]
        if mapped.get("gender"):
            data["gender"] = mapped["gender"].upper()
        if mapped.get("semester"):
            try:
                data["semester"] = coerce_int(mapped["semester"], "semester")
            except ValueError as e:
                row_errors.append(str(e))
        if mapped.get("year_of_study"):
            try:
                data["year_of_study"] = coerce_int(mapped["year_of_study"], "year_of_study")
            except ValueError as e:
                row_errors.append(str(e))

        if row_errors:
            errors.append(BulkUploadRow(row_number=row_num, status=BulkRowStatus.ERROR, data=data, errors=row_errors))
        elif existing:
            to_update.append(BulkUploadRow(row_number=row_num, status=BulkRowStatus.UPDATE, data=data, existing_id=str(existing.id)))
        else:
            to_create.append(BulkUploadRow(row_number=row_num, status=BulkRowStatus.CREATE, data=data))

    return BulkUploadPreview(total_rows=len(rows), to_create=to_create, to_update=to_update, errors=errors)


async def execute_import(
    db: AsyncSession,
    rows: list[BulkUploadRow],
    institution_id: uuid.UUID,
    actor_user_id: uuid.UUID | None,
    actor_role: str | None,
) -> BulkImportResult:
    from app.services.user_service import hash_password
    from app.models.audit_log import AuditAction
    from app.services import audit_log_service

    imported = 0
    updated = 0
    failed = 0
    fail_errors: list[BulkUploadRow] = []

    for row in rows:
        if row.status == BulkRowStatus.ERROR:
            failed += 1
            fail_errors.append(row)
            continue

        try:
            async with await db.begin_nested():
                dept_id = uuid.UUID(row.data["department_id"]) if row.data.get("department_id") else None
                email = row.data.get("email", "")
                name = row.data.get("name", "")
                enroll = row.data.get("enrollment_number", "")

                if row.status == BulkRowStatus.CREATE:
                    hashed = hash_password(STUDENT_DEFAULT_PASSWORD)
                    user = User(
                        full_name=name,
                        email=email,
                        hashed_password=hashed,
                        role=UserRole.STUDENT,
                        department_id=dept_id,
                        institution_id=institution_id,
                        phone=row.data.get("phone"),
                        gender=row.data.get("gender"),
                    )
                    db.add(user)
                    await db.flush()

                    profile = StudentProfile(
                        user_id=user.id,
                        enrollment_number=enroll,
                        degree_type=row.data.get("degree_type", ""),
                        program=row.data.get("program", ""),
                        semester=row.data.get("semester"),
                        year_of_study=row.data.get("year_of_study"),
                    )
                    db.add(profile)
                    await db.flush()

                    from app.services.student_service import auto_allocate_profile_if_timetable_published
                    await auto_allocate_profile_if_timetable_published(db, profile, commit=False)

                    await audit_log_service.record(
                        db, actor_user_id=actor_user_id, actor_role=actor_role,
                        action=AuditAction.CREATED, entity_type="Student",
                        entity_id=profile.id, entity_label=enroll,
                        institution_id=institution_id,
                    )
                    imported += 1

                elif row.status == BulkRowStatus.UPDATE and row.existing_id:
                    profile_id = uuid.UUID(row.existing_id)
                    result = await db.execute(select(StudentProfile).where(StudentProfile.id == profile_id))
                    profile = result.scalar_one_or_none()
                    if not profile:
                        failed += 1
                        fail_errors.append(BulkUploadRow(row_number=row.row_number, status=BulkRowStatus.ERROR, data=row.data, errors=["Student profile not found"]))
                        continue

                    if row.data.get("degree_type"):
                        profile.degree_type = row.data["degree_type"]
                    if row.data.get("program"):
                        profile.program = row.data["program"]
                    if row.data.get("semester") is not None:
                        profile.semester = row.data["semester"]
                    if row.data.get("year_of_study") is not None:
                        profile.year_of_study = row.data["year_of_study"]

                    result = await db.execute(select(User).where(User.id == profile.user_id))
                    user = result.scalar_one_or_none()
                    if user:
                        if name:
                            user.full_name = name
                        if dept_id:
                            user.department_id = dept_id
                        if row.data.get("phone") is not None:
                            user.phone = row.data.get("phone")
                        if row.data.get("gender") is not None:
                            user.gender = row.data.get("gender")

                    await db.flush()

                    from app.services.student_service import auto_allocate_profile_if_timetable_published
                    await auto_allocate_profile_if_timetable_published(db, profile, commit=False)

                    await audit_log_service.record(
                        db, actor_user_id=actor_user_id, actor_role=actor_role,
                        action=AuditAction.UPDATED, entity_type="Student",
                        entity_id=profile.id, entity_label=enroll or profile.enrollment_number,
                        institution_id=institution_id,
                    )
                    updated += 1

        except Exception as e:
            failed += 1
            fail_errors.append(BulkUploadRow(row_number=row.row_number, status=BulkRowStatus.ERROR, data=row.data, errors=[str(e)]))

    await db.commit()
    return BulkImportResult(imported=imported, updated=updated, failed=failed, errors=fail_errors)
