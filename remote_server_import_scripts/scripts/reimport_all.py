...
def _execute_values(cur, sql: str, rows: list, template: str | None = None) -> None:
    ...
    if _PSYCOPG_VER == 2:
        ...
        # Simpler: build one big multi-row INSERT with positional params
        if not rows:
            return
        # Replace "VALUES %s" with "VALUES " + placeholders
        placeholders = template if template else f"({','.join(['%s'] * len(rows[0]))})"
        # Build multi-row insert
        flat: list = []
        value_placeholders: list[str] = []
        for row in rows:
            value_placeholders.append(placeholders.replace("%s", "%s"))
            flat.extend(row)
...
# S57 letter-slot PE pattern: two-digit year + single letter A–D + trailing digits
# e.g. AE23A12, CH23D11, IT23B14  (regular courses: AE23531 — only digits after year)
_PE_LETTER_SLOT_RE = re.compile(r'\d{2}[A-D]\d+$')
...
def _elective_type(code: str) -> str | None:
    ...
    if "OE" in cu:
        return "OPEN"
    # S57 convention: letter-slot codes like AE23A12, CH23D11 are PE courses
    if _PE_LETTER_SLOT_RE.search(cu):
        return "PROFESSIONAL"
    return None


def _pe_slot_label(code: str) -> str | None:
    """Return the slot letter ('A'–'D') for S57 letter-slot PE codes, else None.

    e.g. 'AE23A12' → 'A', 'CH23D11' → 'D', 'AE23PE31' → None (legacy)
    """
    m = _PE_LETTER_SLOT_RE.search(code.upper())
    if m:
        letter_match = re.search(r'[A-D]', m.group())
        return letter_match.group() if letter_match else None
    return None
        ...
        for df in (theory_df, lab_df):
            for _, r in df.iterrows():
                ...
                if norm and norm not in seen_norm:
                    seen_norm.add(norm)
                    subject_names.append(raw_name)

        pairs: list[tuple[str, str]] = []
        if len(subject_names) == len(slot_codes):
            pairs = list(zip(subject_names, slot_codes))
        elif subject_names and slot_codes:
            pairs = list(zip(subject_names, slot_codes))
...
def _max_seats_from_row(r, is_lab: bool) -> int | None:
    ...
    if cap_info and "/" in cap_info:
        try:
            mx = int(float(cap_info.rsplit("/", 1)[-1].strip()))
            if mx > 0:
                return mx
...
def step0_wipe(cur, scenario_id: str, term_id: str, dry_run: bool) -> None:
    ...
    if dry_run:
        ...
        print(f"  (dry) Would delete: {n} sessions, {nb} offering_buckets, {nt} COHORT targets, "
              f"{ns} student selections, {nep} elective_pools, {nr} Redis seat keys")
        return
...
def step1_entities(cur, lab_df: pd.DataFrame, theory_df: pd.DataFrame,
    ...
    for r in cur.fetchall():
        if r["staff_code"]:
            existing_by_code[r["staff_code"].strip().lower()] = r["id"]
    ...
    pe_name_maps = build_pe_name_to_slot_maps(lab_df, theory_df, pe_pools)
        ...
        if not dry_run:
            cur.execute(
                ...
                (
                    ...
                    dept_id, info["sem"], _elective_type(code),
                ),
            )
        existing_courses.add(code)
        courses_created += 1
    print(f"  Courses: {courses_created} created")
...
def step2_import(cur, lab_df: pd.DataFrame, theory_df: pd.DataFrame,
    ...
    for r in _fac_rows_s2:
        ...
        if r["staff_code"]:
            faculty_lk[r["staff_code"].strip().lower()] = str(r["id"])
...
def step3_cohorts(cur, lab_df: pd.DataFrame, theory_df: pd.DataFrame,
    ...
    for r in _fac_rows_s3:
        if r["staff_code"]:
            fac_by_code[r["staff_code"].strip().lower()] = r["id"]
    ...
    for df, is_lab_df in [(theory_df, False), (lab_df, True)]:
        for _, r in df.iterrows():
            ...
            def _register_offering(bn: int | None) -> None:
                ...
                prev_cap = cohort_offering_caps.get(cap_key)
                cohort_offering_caps[cap_key] = (
                    max(prev_cap, row_cap)
                    if (prev_cap is not None and row_cap is not None)
                    else (row_cap if row_cap is not None else prev_cap)
                )
    ...
    for (dept, sem, gname) in groups_data:
        dept_sem_groups[(dept, sem)].append(gname)
...
def step4_pe_oe_pools(cur, lab_df: pd.DataFrame, theory_df: pd.DataFrame,
    ...
    for df, is_lab in [(theory_df, False), (lab_df, True)]:
        for _, r in df.iterrows():
            ...
            for batch_num in batch_nums:
                entry_key = (pool_key, course_id, fac_id or "", ta_id, batch_num)
                if entry_key not in seen_pool_entries:
                    seen_pool_entries.add(entry_key)
                    pools[pool_key].append((course_id, fac_id, ta_id, slot_code, cname, batch_num))
                    pool_entry_meta[entry_key] = {"max_seats": row_cap}
                elif row_cap is not None:
                    prev = pool_entry_meta.get(entry_key, {}).get("max_seats")
                    pool_entry_meta[entry_key]["max_seats"] = (
                        max(prev, row_cap) if prev is not None else row_cap
                    )
    ...
    for pool_key, entries in sorted(pools.items()):
        ...
        for g_idx, (course_id, fac_id, ta_id, code, cname, batch_num) in enumerate(entries, 1):
            offering_id = str(uuid.uuid4())
            offering_group = cohort_g if cohort_g is not None else g_idx
            entry_key = (pool_key, course_id, fac_id or "", ta_id, batch_num)
            max_seats = pool_entry_meta.get(entry_key, {}).get("max_seats")
            ...
            if not dry_run:
                cur.execute(
                    ...
                    (offering_id, bucket_id, course_id, fac_id or None,
                     offering_group, sem, max_seats, batch_num),
                )
                if ta_id and batch_num is not None:
                    cur.execute(
                        """UPDATE scheduled_sessions
                           ...
                           WHERE scenario_id = %s AND course_id = %s
                             AND session_id LIKE %s
                             AND offering_id IS NULL""",
                        (offering_id, scenario_id, link_course_id, f"{ta_id}_%_B{batch_num}"),
                    )
                elif ta_id:
                    cur.execute(
                        """UPDATE scheduled_sessions
                           ...
                           WHERE scenario_id = %s AND course_id = %s
                             ...
                             AND session_id !~ '_B[0-9]+$'
                             AND offering_id IS NULL""",
                        (offering_id, scenario_id, link_course_id, f"{ta_id}_%"),
                    )
                elif fac_id:
                    cur.execute(
                        """UPDATE scheduled_sessions
