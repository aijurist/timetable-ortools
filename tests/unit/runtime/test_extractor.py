...
from src.runtime.solver_schema import BoundEvent, SolverResult


def _build_extended_container() -> ExtendedDataContainer:
...
def _build_constraint_model() -> ConstraintModel:
	data = _build_extended_container()
	...
	theory_block = TheoryVariableBlock(
		group_timeslots={"G1": {0: {1: theory_var}}},
		requirements={
...
def _build_solver_result(model: cp_model.CpModel) -> SolverResult:
...
def test_schedule_extractor_builds_and_exports(tmp_path: Path) -> None:
	...
	solver_result = _build_solver_result(constraint_model.model)
	...
	assert result.theory_entries[0].five_pm_flag is True
