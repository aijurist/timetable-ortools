...
class LabScheduler:
    ...
    def process_lab_requirements(self):
        """Identify courses that need lab allocation and their requirements."""
        ...
        for _, course_row in lab_courses.iterrows():
            ...
            # Fixed calculation: divide by 2 and round up
            required_sessions = max(1, (practical_hours + 1) // 2)  # Round up division
            
            # For exact mapping:
            if practical_hours == 1:
                required_sessions = 1  # 1 hour = 1 session (rounded up)
            elif practical_hours == 2:
                required_sessions = 1  # 2 hours = 1 session
            elif practical_hours == 3:
                required_sessions = 2  # 3 hours = 2 sessions (rounded up)
            elif practical_hours == 4:
                required_sessions = 2  # 4 hours = 2 sessions
            elif practical_hours == 5:
                required_sessions = 3  # 5 hours = 3 sessions (rounded up)
            elif practical_hours == 6:
                required_sessions = 3  # 6 hours = 3 sessions
            else:
                required_sessions = max(1, (practical_hours + 1) // 2)  # General formula
            
            # DEFAULT ALL STUDENT COUNTS TO 70 (ignore actual student count and lab capacity)
            default_student_count = 70
            total_sessions_needed = required_sessions  # Don't multiply by batches - simplified
            
            # Track course instances for this teacher
            ...
            self.lab_requirements[teacher_id].append({
                ...
                'display_course_code': display_code,  # For visualization differentiation
                'course_suffix': course_suffix,  # S1, S2, etc.
                'course_name': course_row['course_name'],
                'practical_hours': practical_hours,
                'student_count': default_student_count,  # Always use 70 students
                'required_sessions': required_sessions,
                'num_batches': 1,  # Simplified - always 1 batch regardless of student count
        ...
        total_sessions = sum([sum(course['total_sessions_needed'] for course in courses) 
                             for courses in self.lab_requirements.values()])
        
        self.logger.info(f"Found {total_courses} courses requiring lab allocation")
        self.logger.info(f"Total lab sessions needed: {total_sessions}")
        self.logger.info(f"Available lab rooms: {len(self.labs)}")
        self.logger.info(f"Lab sessions per week: {len(self.lab_sessions) * self.num_days}")
        self.logger.info(f"Total lab capacity per week: {len(self.labs) * len(self.lab_sessions) * self.num_days}")
        self.logger.info(f"NOTE: All student counts defaulted to 70, no lab capacity constraints applied")
        
        # Log detailed requirements for debugging
        for teacher, courses in self.lab_requirements.items():
            for course in courses:
                self.logger.info(f"Course {course['display_course_code']} (ID: {course['course_instance_id']}, Teacher {teacher}): {course['practical_hours']} practical hours -> {course['required_sessions']} lab sessions")
                        ...
                        if solver.Value(lab_assignments[course_instance_id][day_idx][session_idx][room_id]) == 1:
                            ...
                            for slot_idx, time_slot in enumerate(session_time_slots):
                                ...
                                lab_schedule_data.append({
                                    ...
                                    'display_course_code': course_info.get('display_course_code', course_info['course_code']),  # S1, S2 differentiation
                            ...
                            self.logger.info(f"Lab assignment: Course {course_info.get('display_course_code', course_info['course_code'])} (ID: {course_info['course_instance_id']}, Teacher {teacher}) -> {session_name} ({session_time_slots[0]} to {session_time_slots[1]}) in Lab {room_details['room_number']}")
    ...
    def combine_theory_and_lab_schedules(self, lab_schedule_data):
        ...
        for lab_entry in lab_schedule_data:
            lab_schedule_entry = {
                ...
                'student_count': lab_entry['student_count'],
